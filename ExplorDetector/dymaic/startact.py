...
from tools.FillInBlank import fillInBlank
...
def run(project, device, source_screen, fragment):
    ...
    while len(widget_stack) != 0:
        print("[++++++++++++++++++]")
        # zyx
        restartScreen1(project, source_screen, device, GptTextSet)
        ...
        # 对新的Screen进行截图
        ###
        cmd0 = source_screen.adb
        shot_dir = getshot.shot(device.uiauto, project, screenvector, cmd0)
        ...
        # zyx
        with open(project.clickAct, "a") as f:
...
def restartScreen1(project, source_screen, device, GptTextSet):
    """
    :param device:
    :param project: 项目对象
    :param screen: 场景对象
    ...
    while True:
        ...
        if texactivity in result.decode("utf8"):
            print("[+] start Act !")
            # zyx
            for widget in GptTextSet:
                widget.set_text(GptTextSet[widget])
                time.sleep(1)
            break
        else:
            print("[-] can't start: ", texactivity)
